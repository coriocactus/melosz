...
export function ComparisonView(el: HTMLElement) {
  ...
  el.addEventListener(
    "renderComparison",
    ((e: CustomEvent<[Option, Option] | null>) => {
  ...
  function update() {
    ...
    if (currentPair) {
      ...
      button1.className =
        "option-button px-6 py-3 bg-blue-500 text-white rounded hover:bg-blue-600 transition duration-150 ease-in-out";
      ...
      button2.className =
        "option-button px-6 py-3 bg-green-500 text-white rounded hover:bg-green-600 transition duration-150 ease-in-out";
  ...
  function handleChoice(winnerId: OptionId, loserId: OptionId) {
    ...
    el.dispatchEvent(
      new CustomEvent("comparisonMade", {
