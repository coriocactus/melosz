...
spec = describe "Marshal" $ do
  ...
  describe "setupOption" $ do
    ...
    it "adds new comparison pairs and GlickoPlayer to existing users' UserState" $ do
      ...
      let finalUserState = stateUserStates finalState Map.! testUser1
          ...
          glickoPlayers = userGlickoPlayers finalUserState
          expectedPair = makeCanonicalPair optA optB

      Set.member expectedPair uncompared `shouldBe` True
      Set.size uncompared `shouldBe` 1

      Map.lookup (optionId optA) glickoPlayers `shouldBe` Just initialGlickoPlayer
      Map.lookup (optionId optB) glickoPlayers `shouldBe` Just initialGlickoPlayer
      Map.size glickoPlayers `shouldBe` 2

    it "does not add pairs or players if no users exist" $ do
  ...
  describe "setupUser" $ do
    ...
    it "initializes UserState correctly based on existing options" $ do
      let startOpts = Set.fromList [optA, optB]
          ...
          expectedGlickoMap = Map.fromSet (const initialGlickoPlayer) (Set.map optionId startOpts)

      finalState <- execAppTest (setupUser testUser1) Nothing startState
      let userState = stateUserStates finalState Map.! testUser1

      userGlickoPlayers userState `shouldBe` expectedGlickoMap
